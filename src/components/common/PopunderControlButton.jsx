...
export function PopunderControlButton() {
  ...
  const handlePauseAds = async () => {
    if (!user) {
      await showConfirmation(
        'Login Required',
        'Please login to pause ads',
        { isSingleButton: true, confirmText: 'OK' }
      );
      return;
    }
  ...
  return (
    <button
      ...
      className="flex items-center gap-2 px-4 py-2.5 rounded-full border transition-all duration-200 active:scale-95 bg-red-900/20 border-red-500/20 hover:bg-red-800/30"
